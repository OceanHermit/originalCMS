...
- `public/index.html` — エディタ本体(ローカルのZIP書き出し/読み込みに加え、Googleログイン・クラウド保存/読み込み・プロジェクト一覧を実装済み)
...
デプロイ後、エディタ画面の右上に以下が表示されます:

- **Googleでログイン** — `/api/auth/login` に遷移してGoogleログイン。ログイン後はメールアドレスと
  「ログアウト」ボタンが表示されます。
- **☁ クラウド保存** — 現在のキャンバスをサーバーに保存(初回はプロジェクト名を入力、以降は上書き)。
  未ログイン時はログインを促します。
- **☁ クラウドを開く** — 保存済みプロジェクトの一覧を表示。クリックで読み込み、🗑 で削除。
- **💾 一時保存** — このブラウザ内に保存(Claude上では専用ストレージ、通常のブラウザでは
  localStorageを使用)。ログイン不要ですが、そのブラウザでしか復元できません。

クラウド保存/読み込みは手順2〜6のD1・R2・Google OAuth・環境変数の設定が完了している必要があります。
